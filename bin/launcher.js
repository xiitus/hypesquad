...
export const prependPathEntries = (env, entries) => {
  ...
  return {
    ...env,
    PATH: [...additions, ...currentEntries].join(":"),
  };
};
...
export const ensureNpxAvailable = ({
  env,
  platform,
  log = console.error,
  exists = commandExists,
  ...
  if (exists({ command: "npx", env: nextEnv, platform })) {
    return nextEnv;
  }

  log("`npx` not found. Installing Node.js...");
  ...
  if (!exists({ command: "npx", env: nextEnv, platform })) {
    throw new Error("Node.js installation completed, but `npx` is still unavailable.");
  }
...
export const launchCli = ({
  ...
  const nextEnv = ensureNpxAvailable({
    env,
    platform,
    log,
    exists,
